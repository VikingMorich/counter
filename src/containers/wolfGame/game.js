import { connect } from 'react-redux';
import game from '../../components/game';
import { push } from 'connected-react-router';
import { restartVariables, updatePlayerIndex } from '../../actions/creators/wolfGame';

const mapStateToProps = (state) => {
    return {
        startGame: state.wolfGame.start,
        activeCharacters: state.wolfGame.activeCharacters,
        playerIndex: state.wolfGame.playerIndex,
        players: state.wolfGame.players,
        playerRols: state.wolfGame.playerRols
    }
}

const mapDispatchToProps = (dispatch) => ({
    backToHome: () => dispatch(push('/wolf-game')),
    restartVariables: () => dispatch(restartVariables()),
    updatePlayerIndex: (index) => dispatch(updatePlayerIndex(index)),
    finish: () => dispatch(push('/finish')),
})

export default connect(mapStateToProps, mapDispatchToProps)(game);